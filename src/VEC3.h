...
public:
  ...
  // Type cast operators 
  operator Vector3Dim<float>();
  operator Vector3Dim<uint16_t>();

  // user defined cast to float array
    operator Scalar*()  {
        return &value[0];
    }
...
template<class Scalar>
inline Vector3Dim<Scalar>::Vector3Dim( Vec4f v )
{
  value[0] = vfget_x(v);
  value[1] = vfget_y(v);
  value[2] = vfget_z(v);
...
template<class Scalar>
inline Vector3Dim<Scalar>
operator*(Scalar s, Vector3Dim<Scalar>& v) 
{
  return Vector3Dim<Scalar>(v[0]*s, v[1]*s, v[2]*s);
}
...
/*************************************************************************
  ...
  */
template<class Scalar>
inline double dot(const Vector3Dim<Scalar> &t, const Vector3Dim<Scalar> &v )
{
  //return t.value[0]*v.value[0] + t.value[1]*v.value[1] + t.value[2]*v.value[2];
  return ((t[0]*(double)v[0]) + (t[1]*(double)v[1]) + (t[2]*(double)v[2]));
}
...
{
  Scalar l = v[0]*(double)v[0] + v[1]*(double)v[1] + v[2]*(double)v[2];
...
/*************************************************************************
  ...
  */
template<class Scalar>
inline Scalar normalizeOld( VECTOR_TYPE &v) 
{
  Scalar norm;
  Scalar l = v[0]*v[0] + v[1]*v[1] + v[2]*v[2];  
  if (fabs(l-1.) < VECTOR_EPSILON*VECTOR_EPSILON) {
...
{
  double norm;
  double l = v[0]*(double)v[0] + v[1]*(double)v[1] + v[2]*(double)v[2];  
  if (fabs(l-1.) < VECTOR_EPSILON*VECTOR_EPSILON) {
    norm = 1.;
	} else if (l > VECTOR_EPSILON*VECTOR_EPSILON) {
    norm = sqrt(l);
...
// Type cast operators 
template<> inline
Vector3Dim<uint16_t>::operator Vector3Dim<float>() { return Vector3Dim<float>( value[0], value[1], value[2] ); }
template<> inline
Vector3Dim<float>::operator Vector3Dim<uint16_t>() 
{ 
  #ifdef UT_ASSERT_LEVEL_2
  for(int k=0;k<3;k++)
  {
    UT_ASSERT0(!(value[k]<-0.1f || value[k]>65535.1f));
  }
  #endif

  return Vector3Dim<uint16_t>(  
      		std::max(0.f,std::min(65535.f,value[0])),
      		std::max(0.f,std::min(65535.f,value[1])),
      		std::max(0.f,std::min(65535.f,value[2]))); 
}
...
inline Vec4f loadVec4f( Vector3Dim<float> &v )
{
  return Vec4f().load_partial(3,v.getAddress());
}

inline void storeVec4f( Vec4f& v4f, Vector3Dim<float> &v )
{
  v4f.store_partial(3,v.getAddress());
}
...
typedef Vector3Dim<uint16_t> Vec3Uint16; 
...
typedef Vector3Dim<double> Vec3Double;
